...
def train(
    ...
    if resume:
        ...
        # Set optimizer
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr0)
    ...
    for epoch in range(epochs):
        ...
        print("\nEpoch: ", epoch)
        for i, (imgs, targets) in tqdm(enumerate(dataloader), ascii=True, total=len(dataloader)):
            ...
            s = ('%8s%12s' + '%10.3g' * 14) % (
                '%g' % (epoch), '%g/%g' % (i, len(dataloader) - 1), rloss['x'],
