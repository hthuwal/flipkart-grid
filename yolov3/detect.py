...
def detect(
    ...
    print("Detecting...")
    for i, (img_paths, img) in tqdm(enumerate(dataloader), ascii=True, total=len(dataloader)):
    ...
    print("%d/%d detected." % (len(imgs), len(dataloader)))
    # Bounding-box colors
    ...
    for img_i, (path, detections) in tqdm(enumerate(zip(imgs, img_detections)), ascii=True, total=len(imgs)):
